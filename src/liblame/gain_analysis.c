...
#if USE_MEMORY_HACK
            if (ival >= rgData->size_A / sizeof(*(rgData->A)))
                ival = rgData->size_A / sizeof(*(rgData->A)) - 1;
#else
...
#if USE_MEMORY_HACK
    retval = analyzeResult(rgData->A, rgData->size_A / sizeof(*(rgData->A)));

    for (i = 0; i < rgData->size_A / sizeof(*(rgData->A)); i++) {
